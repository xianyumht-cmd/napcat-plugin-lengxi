...
> 说明：本规划不包含对平台内部机制的逆向或绕过，仅基于插件运行期可观测信号做自适应降载与策略收敛。

### 6.1 目标

- 在高风险时段降低触发风控概率，提升消息送达稳定性与账号安全性。
- 把“经验调参”升级为“数据驱动”的实时策略系统。

### 6.2 观测面（实时）

- 发送面：队列长度、每分钟发送量、限流等待时长、熔断触发次数。
- 接口面：`send_group_msg`/`send_private_msg` 失败率、错误码分布、重试成功率。
- 业务面：高频命令来源、群维度触发密度、模板命中分布、scene 命中分布。
- 账号面：人工登记的异常事件（临时限制、功能受限、验证码干预）。

### 6.3 策略面（自适应）

- L0 正常：按现有并发和概率策略运行。
- L1 预警：降低并发与回复概率，启用更保守模板策略。
- L2 风险：按群限流，关闭非关键自动回复，只保留管理核心功能。
- L3 保护：进入保护窗，仅响应高优先级指令与人工确认命令。

### 6.4 执行路线

- 第 1 周：补齐指标采集与日志结构化，形成风控观测报表。
- 第 2 周：上线风险等级评估器（规则版），接入发送策略开关。
- 第 3 周：按群/按命令做差异化阈值，支持 WebUI 实时查看与手动降级。
- 第 4 周：复盘误报与漏报，固化默认阈值与应急 SOP。

### 6.5 验收指标

- 发送失败率下降 30%+（对比改造前 7 天）。
- 熔断触发后恢复时间降低 40%+。
- 高峰时段的命令可用率达到 99%。
- 人工干预次数下降 50%+。
