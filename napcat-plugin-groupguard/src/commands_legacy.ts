...
async function sendGroupScene(groupId: string, scene: string, fallback: string, vars?: Record<string, string | number | boolean>): Promise<void> {
  await pluginState.sendGroupText(groupId, fallback, { scene, vars, force: true, applyTemplate: true });
}

async function sendPrivateScene(userId: string, scene: string, fallback: string, vars?: Record<string, string | number | boolean>): Promise<void> {
...
发送“我的积分”查看余额`;
  ...
  if (text.startsWith('兑换 ')) {
      ...
      if (item === '免死金牌') {
          ...
          userSignin.points -= cost;
          await dbQuery.runInTransaction(() => {
            dbQuery.updateSignin(groupId, userId, userSignin);
            dbQuery.setWarning(groupId, userId, 0);
          });
  ...
  for (const qa of qaList) {
    ...
    if (matched) {
      ...
      if (reply.includes('[CQ:')) {
         await pluginState.sendGroupMsg(groupId, [{ type: 'text', data: { text: reply } }], { force: false, applyTemplate: true });
      } else {
         await pluginState.sendGroupText(groupId, reply, { force: false, applyTemplate: true });
      }
